<nav class="topnav navbar navbar-expand shadow navbar-light bg-white" id="sidenavAccordion">
    ...
    <ul class="navbar-nav align-items-center ml-auto">
        ...
        <!--User Registration + New Comment Notification-->
            <?php
                $sql = "SELECT * FROM message where ms_state =:state";
                $stmt= $pdo->prepare($sql);
                $stmt->execute([
                    ':state'=>0
                ]);
                $msRows = $stmt->rowCount();
            ?>
        ...
        <li class="nav-item dropdown no-caret mr-3 dropdown-notifications">
            <a class="btn btn-icon btn-transparent-dark dropdown-toggle" id="navbarDropdownMessages" href="#" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                ...
                <?php  
                if($msRows != 0){ ?>
                    <span class="badge badge-red"><?php echo $msRows; ?></span>
               <?php }
                ?>
            ...
            <div class="dropdown-menu dropdown-menu-right border-0 shadow animated--fade-in-up" aria-labelledby="navbarDropdownMessages">
                ...
                <a class="dropdown-item dropdown-notifications-item" href="#"
                    >
                    <?php 
                    while ($sm = $stmt->fetch(PDO::FETCH_ASSOC)) {
                        $ms_details = substr($sm['ms_details'], 0, 30);
                        $ms_name = $sm['ms_username'];
                        $sm_date = $sm['ms_date']; ?>
                            <div class="dropdown-notifications-item-content">
                                <div class="dropdown-notifications-item-content-text">
                                    <?php echo $ms_details; ?>
                                ...
                                <div class="dropdown-notifications-item-content-details">
                                    <?php echo $ms_name; ?> &#xB7; <?php echo $sm_date; ?>
                                </div>
                             </div>
                        <?php } ?>
